...
import org.kodj.core.listeners.BlockChainListener;
import org.kodj.store.FlatDB;
import org.kodj.store.HashStore;
import org.kodj.store.MasternodeDB;
 ...
 * kodj for a specific network. You can get an instance of this class through calling {@link #get()}.</p>
 *
...
public class Context {
    ...
    public Context(NetworkParameters params) {
        log.info("Creating kodj {} context.", VersionMessage.BITCOINJ_VERSION);
     ...
     * independently use kodj (or possibly alt coin forks of it) can operate correctly.
     *
    ...
    public static Context get() {
        ...
        if (tls == null) {
            if (isStrictMode) {
                log.error("Thread is missing a kodj context.");
                log.error("You should use Context.propagate() or a ContextPropagatingThreadFactory.");
                throw new IllegalStateException("missing context");
            }
            if (lastConstructed == null)
                throw new IllegalStateException("You must construct a Context object before using kodj!");
            slot.set(lastConstructed);
            log.error("Performing thread fixup: you are accessing kodj via a thread that has not had any context set on it.");
    ...
    /**
     * Require that new threads use {@link #propagate(Context)} or {@link org.kodj.utils.ContextPropagatingThreadFactory},
     ...
     * a {@link org.kodj.utils.ContextPropagatingThreadFactory}.
     */
    ...
    /**
     * Returns the {@link org.kodj.core.NetworkParameters} specified when this context was (auto) created. The
    ...
    BlockChainListener updateHeadListener = new BlockChainListener () {
        ...
        {
            //todo furszy: commented kod MN stuff
